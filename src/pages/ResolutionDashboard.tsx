...
const ResolutionDashboard = () => {
  ...
  // Mock function to check module access
  const hasModuleAccess = (module: string) => {
    // Mock implementation - in real app this would check user's subscription
    const mockSubscriptions = ['vdr', 'litigation', 'compliance'];
    return mockSubscriptions.includes(module);
  };
  ...
  const handleSignAndSaveRFRP = async (signatureType: 'digital' | 'electronic') => {
    try {
      ...
      toast({ title: 'RFRP Created', description: `RFRP signed with ${signatureType === 'digital' ? 'Digital Signature (DSC)' : 'E-Signature'} and  Email has been sent to all PRAs. RFRP link is provided in the same email to upload the resolution plan.` });
    } finally {
      setSigning(false);
    }
  };
